...
function expandedParams(params) {
  ...
  if (ld.includes(paramKeys, "apogee") && ld.includes(paramKeys, "perigee")) {
    params.semimajorAxis = semimajorAxisFromApogeeAndPerigee(params.apogee, params.perigee);
    params.eccentricity = eccentricityFromSemimajorAxisAndPerigee(params.semimajorAxis, params.perigee);
...
function semimajorAxisFromApogeeAndPerigee(apogee, perigee) {
  return ((constants.EARTH_RADIUS * 2) + apogee + perigee) / 2.0;
}

function eccentricityFromSemimajorAxisAndPerigee(semimajorAxis, perigee) {
  return (semimajorAxis / (constants.EARTH_RADIUS + perigee)) - 1;
}
